...
const TaskList = ({ children }: React.PropsWithChildren) => {
  ...
  const { data: stages, isLoading: isLoadingStages, error: errorStages } = useList<TaskStage>({
    resource: 'taskStages',
    filters: [
      {
        field: 'title',
  ...
  const { data: tasks, isLoading: isLoadingTasks, error: errorTasks } = useList<GetFieldsFromList<TasksQuery>>({
    resource: 'tasks',
    sorters: [
      {
        field: 'dueDate',
  ...
  console.log('TaskList rendered'); // Debugging

  if (isLoading) return <PageSkeleton />;
  if (errorStages || errorTasks) return <div>Error loading data!</div>;
